  ...
  <div class="min-h-screen bg-gray-100 flex items-center justify-center p-4">
    <div class="bg-white p-10 rounded-xl shadow-lg w-full max-w-[450px]">
      ...
      <form @submit.prevent="login" class="space-y-4">
        ...
        <button
          class="w-full bg-blue-600 text-white font-semibold py-3 rounded-md hover:bg-blue-700 transition mt-4"
        >
          로그인
        </button>
      </form>
      ...
      <p class="text-center text-sm text-gray-500 mt-8">
        계정이 없으신가요?
        <RouterLink :to="{ name: 'signup' }" class="text-blue-600 hover:underline font-medium"
          >회원가입</RouterLink
        >
      </p>
    </div>
